...
// Minimal PebbleKit DataLogging test app for exercising `stoandl` datalog capture (TESTING.md §5.8).
// Logs an incrementing 4-byte little-endian uint every few seconds to tag 42, shows a live counter,
// and forces a flush to the phone on SELECT (data logging otherwise spools and syncs lazily).
...
static void tick(void *ctx) {
  uint32_t v = ++s_value;
  DataLoggingResult r = data_logging_log(s_session, &v, 1);  // 1 item of sizeof(uint32_t) bytes
  snprintf(s_buf, sizeof(s_buf), "logged: %lu\nresult: %d\n\nSELECT = flush", (unsigned long)v, (int)r);
  text_layer_set_text(s_label, s_buf);
  APP_LOG(APP_LOG_LEVEL_INFO, "logged %lu (result=%d)", (unsigned long)v, (int)r);
  s_timer = app_timer_register(INTERVAL_MS, tick, NULL);
}

static void select_click(ClickRecognizerRef ref, void *ctx) {
  // Close the session to flush its buffered items to the phone now, then open a fresh one.
  data_logging_finish(s_session);
  s_session = data_logging_create(LOG_TAG, DATA_LOGGING_UINT, sizeof(uint32_t), false);
