...
- [ ] **Watch screenshot** — `stoandl screenshot` via libpebble3's `ConnectedPebble.Screenshot.takeScreenshot()`. Handy for sharing watchfaces and bug reports; the official app has it.
...
- [ ] **Factory reset / reset to recovery** — expose `ConnectedPebble.Debug.factoryReset()` and `resetIntoPrf()` (boot into recovery/PRF firmware). Natural companion to the firmware tooling for un-bricking and clean handoff.
- [ ] **Language packs** — install a non-English firmware language pack onto the watch via `ConnectedPebble.LanguageInstall`. The official app offers this; stoandl would need a (public) language-pack source.
- [ ] **Per-app notification settings** — wire `NotificationAppsSync` (currently no-op) so the set of notifying apps syncs to the watch, enabling the watch's per-app mute/notification preferences. Complements the existing host-side `notification.blocklist`.
