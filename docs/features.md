...
- [ ] **Location to watch apps (geolocation)** — wire the desktop's GeoClue2 location into libpebble3's `SystemGeolocation` hook (currently a JVM no-op), so PKJS apps' `navigator.geolocation` and location-aware watchapps/sports apps get a fix. stoandl already speaks GeoClue for weather — this just reuses that provider for the watch-facing hook. Low-effort, high-value gap vs the official app.
