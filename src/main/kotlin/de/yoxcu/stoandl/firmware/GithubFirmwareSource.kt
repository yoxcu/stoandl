...
class GithubFirmwareSource(
    ...
    private data class Release(val tag: String, val assets: List<Asset>)
    ...
    private suspend fun latestRelease(): Release? = withContext(Dispatchers.IO) {
        try {
            if (includePrereleases) {
                ...
                LenientJson.decodeFromString<List<GhRelease>>(body)
                    .firstOrNull { !it.draft }
                    ?.toRelease()
            } else {
                val body = getText("https://api.github.com/repos/$repo/releases/latest")
                    ?: return@withContext null
                LenientJson.decodeFromString<GhRelease>(body).toRelease()
            }
        } catch (e: Exception) {
     ...
     */
    private fun normalBundleFor(release: Release, boardRevision: String): FirmwareBundle? {
    ...
    private data class GhRelease(
        ...
        fun toRelease() = Release(
            tag = tagName,
            assets = assets.map { Asset(it.name, it.browserDownloadUrl) },
        )
    }
